...
**Single-activity deep analysis (on request only).** When the runner asks to break down ONE session, `scripts/analyze_activity.py` (or `ompb_core.analyze_activity("strava-<id>" | "<path>.fit")`) reads that activity's **laps** → workout structure (interval reps + recovery, tempo block, progression, steady) and pacing (negative split / fade / consistency), plus per-second **streams** → aerobic **decoupling** (Pa:HR drift, steady efforts ≥15 min only), **time-in-HR-zone**, a stream-derived **hard-effort count** that resolves auto-lap ambiguity (e.g. a "steady" lap-labelled run that is really 8 intervals), and **GAP** (grade-adjusted pace, when altitude/grade is present — Strava always; many `.fit` exports omit altitude → skipped). `.fit` is offline; Strava is up to 2 API calls (detail + streams). NEVER bulk-run it (rate limits). It asserts a high-confidence `type` only for HR-confirmed intervals (and long); easy/tempo intensity stays the calibrated classifier's call. Pass `write=True` (or `--write`) to lock the matched log entry to the analyzed type with `type_source="laps"` so reclassify preserves it. To deliver the breakdown visually, `scripts/build_activity.py` / `ompb_core.build_activity(source, lang)` renders it as a self-contained HTML **card** (workout-shape bars, reps, decoupling/zones/GAP cells) — the session-level companion to `/pb-week` and `/pb-report`.
</data_ingest>
