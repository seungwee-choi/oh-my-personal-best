...
- `type_source` (optional): `"name"` when the type came from an explicit activity name/type keyword (CSV import). `reclassify.py` preserves these and only re-derives types it set itself.
